...
public class PerlEditor extends TextEditor implements IPropertyChangeListener {
	...
	/**
	 * Flag used to avoid updating outline selection in response to a caret move
	 * caused by a previous outline selection update.
	 */
	...
	public void createPartControl(Composite parent) {
		...
		source = new SourceFile(PerlEditorPlugin.getDefault().getLog(),
	...
	public Object getAdapter(Class adapter) {
		if (ProjectionAnnotationModel.class.equals(adapter)) {
			if (this.projectionSupport != null) {
				Object result = this.projectionSupport.getAdapter(sourceViewer,
						adapter);
				if (result != null) {
					return result;
				}
	...
	/**
	 * @return test interface to this editor's internals; do not use outside of
	...
	public void jumpToMatchingBracket() {
		...
		int matchOffset = findMatchingBracket(sourceViewer.getDocument(),
				caretOffset, true);
	...
	/**
	 * Immediately revalidates syntax of the edited file. This method has no
	 * effect if the syntax validation thread is not installed.
	 */
	...
	protected void createActions() {
		...
		wireAction(new ContentAssistAction(PerlEditorMessages
				.getResourceBundle(), "ContentAssistProposal.", this),
				PerlEditorCommandIds.CONTENT_ASSIST,
				PerlEditorActionIds.CONTENT_ASSIST);

		wireAction(new ToggleCommentAction(this),
				PerlEditorCommandIds.TOGGLE_COMMENT,
				PerlEditorActionIds.TOGGLE_COMMENT);
		wireAction(new CritiqueSourceAction(this),
				PerlEditorCommandIds.CRITIQUE_SOURCE,
				PerlEditorActionIds.CRITIQUE_SOURCE);
		wireAction(new FormatSourceAction(this),
				PerlEditorCommandIds.FORMAT_SOURCE,
				PerlEditorActionIds.FORMAT_SOURCE);
		wireAction(new Jump2BracketAction(this),
				PerlEditorCommandIds.MATCHING_BRACKET,
				PerlEditorActionIds.MATCHING_BRACKET);
		wireAction(new ExportHtmlSourceAction(this),
				PerlEditorCommandIds.HTML_EXPORT,
				PerlEditorActionIds.HTML_EXPORT);
		wireAction(new ValidateSourceAction(this),
				PerlEditorCommandIds.VALIDATE_SYNTAX,
				PerlEditorActionIds.VALIDATE_SYNTAX);
		wireAction(new OpenDeclarationAction(this),
				PerlEditorCommandIds.OPEN_SUB, PerlEditorActionIds.OPEN_SUB);
		wireAction(new PerlDocAction(this), PerlEditorCommandIds.PERL_DOC,
				PerlEditorActionIds.PERL_DOC);
		wireAction(new ExtractSubroutineAction(this),
				PerlEditorCommandIds.EXTRACT_SUBROUTINE,
	...
	protected final ISourceViewer createSourceViewer(Composite parent,
		...
		sourceViewer = new PerlSourceViewer(parent, ruler, fOverviewRuler,
	...
	private void caretMoved() {
		...
		if (outlinePage == null || source == null
				|| source.getDocument() == null)
			return;
	...
	private void installBracketInserter() {
		bracketInserter = new PerlBracketInserter(PerlEditorPlugin.getDefault()
				.getLog());
	...
	private void installBracketMatcher() {
		bracketMatcher = new PerlPairMatcher(PerlEditorPlugin.getDefault()
				.getLog());
		bracketMatcher.setViewer(sourceViewer);
	}
	...
	private void installModuleCompletionHelper() {
		...
		Thread backgroundLoader = new Thread(new Runnable() {
			public void run() {
				try {
					ModuleCompletionHelper completionHelper = ModuleCompletionHelper
							.getInstance();
	...
	private void installSyntaxValidationThread() {
		...
		validationThread.setDocument(
				(IResource) ((IAdaptable) getEditorInput())
						.getAdapter(IResource.class), sourceViewer
						.getDocument());
	...
	private void reconfigureBracketInserter() {
		...
		bracketInserter.setCloseBracesEnabled(preferenceStore
	...
	private void wireAction(IAction action, String commandId,
			String perlActionId) {
	...
	public class TestInterface {
		...
		public int getHighlightedBracketOffset() {
			...
			PerlPairMatcher matcher = new PerlPairMatcher(PerlEditorPlugin
					.getDefault().getLog());

			matcher.setViewer(null);
			matcher.match(sourceViewer.getDocument(), offset);

			return offset - 1 == matcher.getStartPos() ? matcher.getEndPos()
					: matcher.getStartPos();
		}
			...
			if (text.length() == 0) {
				...
				if (i == -1)
					throw new RuntimeException("text \"" + text
							+ "\" not found in editor");
				getSelectionProvider().setSelection(
						new TextSelection(i, text.length()));
			}
		}
	}
	...
	protected abstract class WordNavigationAction extends TextNavigationAction {
		...
		public final void run() {
			...
			PerlPartitioner partitioner = (PerlPartitioner) document
		...
		protected abstract void run(ISourceViewer viewer, IDocument document,
				int position, PerlPartitioner partitioner, int docLength);
		...
		protected final void setCaretPosition(final int position) {
			if (select) {
				...
				if (text != null && !text.isDisposed()) {
					...
					final int offset = modelOffset2WidgetOffset(viewer,
							position);

					if (caret == selection.x)
						text.setSelectionRange(selection.y, offset
								- selection.y);
					else
						text.setSelectionRange(selection.x, offset
								- selection.x);
				}
			} else {
	...
	private final class NextWordAction extends WordNavigationAction {
		 ...
		 *            code from {@link org.eclipse.swt.custom.ST}.
		 */
		protected NextWordAction(int code, boolean select) {
			super(code, select);
		}

		protected void run(ISourceViewer viewer, IDocument document,
				int position, PerlPartitioner partitioner, int length) {
			try {
				...
				if (position < length
					...
					ITypedRegion partition = partitioner.getPartition(position,
							true);

					int partitionEnd = partition.getOffset()
							+ partition.getLength();

					if (position == partitionEnd && position < length) {
						partition = partitioner
								.getPartition(position + 1, true);
						partitionEnd = partition.getOffset()
								+ partition.getLength();
					}
			...
			try {
				...
				if (document.getLineOffset(line) == position) {
					...
					if (position == partitionStart && position > 0) {
						partition = partitioner.getPartition(position - 1,
								false);
