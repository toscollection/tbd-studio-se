...
import org.epic.perleditor.actions.CritiqueSourceAction;
...
public class PerlEditor extends TextEditor implements IPropertyChangeListener {
    ...
    /**
     * Flag used to avoid relocating caret in response to an outline selection change triggered by a previous caret
     * move.
     */
    private boolean syncToOutline;

    /**
     * Flag used to avoid updating outline selection in response to a caret move caused by a previous outline selection
     * update.
     */
    ...
    public void createPartControl(Composite parent) {
        ...
        source = new SourceFile(PerlEditorPlugin.getDefault().getLog(), getViewer().getDocument());

        reconcile();
    }

    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs any extra disposal actions required by
     * the Perl editor.
     */
    public void dispose() {
        ...
        try {
            if (sourceViewer instanceof ITextViewerExtension && bracketInserter != null) {
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs any extra revert behavior required by
     * the Perl editor.
     */
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs any extra save behavior required by the
     * Perl editor.
     */
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs any extra save as behavior required by
     * the Perl editor.
     */
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs sets the input of the outline page after
     * AbstractTextEditor has set input.
     */
    public void doSetInput(IEditorInput input) throws CoreException {
        ...
        if (input instanceof ILocationProvider) {
            ...
            if (input == null)
                throw new CoreException(new Status(IStatus.ERROR, PerlEditorPlugin.getPluginId(), IStatus.OK,
                        "Could not open requested file " + l.getPath(l).makeAbsolute()
                                + ". Inspect the Error Log for additional information.", null));
        }
    ...
    /**
     * Provided that the given document contains a bracket-like character at the given offset, returns the offset of the
     * matching (pair) character (if found). Otherwise, returns -1.
     * 
     * @param exact if false, search for the match within currently displayed text only; if true, search in the entire
     * document
     */
    public int findMatchingBracket(final IDocument document, final int offset, boolean exact) {
        if (exact)
        ...
        try {
            ...
            if (matchRegion == null)
                ...
                ret[0] = matchRegion.getOffset() == offset - 1 ? matchRegion.getOffset() + matchRegion.getLength() - 1
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method performs gets the Perl content outline page if
     * request is for a an outline page.
     */
    public Object getAdapter(Class adapter) {
        if (ProjectionAnnotationModel.class.equals(adapter)) {
            if (this.projectionSupport != null) {
                Object result = this.projectionSupport.getAdapter(sourceViewer, adapter);
                if (result != null) {
                    return result;
                }
            }
    ...
    /**
     * @return test interface to this editor's internals; do not use outside of white-box test cases!
     */
    ...
    /**
     * Provided that the caret's current position is after a bracket-like character, jumps to its matching character (if
     * found). Otherwise, this method has no effect.
     * 
     * @see PerlPairMatcher
     */
    public void jumpToMatchingBracket() {
        int caretOffset = sourceViewer.getSelectedRange().x;
        int matchOffset = findMatchingBracket(sourceViewer.getDocument(), caretOffset, true);
    ...
    /**
     * Updates the editor's dependent views and state after a document change. This method is only intended for use by
     * {@link PerlReconcilingStrategy}.
     */
    public void reconcile() {
        ...
        display.syncExec(new Runnable() {
            ...
            public void run() {
                ...
                if (outlinePage != null)
                    outlinePage.updateContent(source);
                if (foldReconciler != null)
                    foldReconciler.reconcile();
                if (tasksReconciler != null)
    ...
    /**
     * Immediately revalidates syntax of the edited file. This method has no effect if the syntax validation thread is
     * not installed.
     */
    public void revalidateSyntax() {
        if (validationThread != null)
            validationThread.revalidate();
    }
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method extend the actions to add those specific to the
     * receiver
     */
    protected void createActions() {
        super.createActions();

        wireAction(new ContentAssistAction(PerlEditorMessages.getResourceBundle(), "ContentAssistProposal.", this),
                PerlEditorCommandIds.CONTENT_ASSIST, PerlEditorActionIds.CONTENT_ASSIST);

        wireAction(new ToggleCommentAction(this), PerlEditorCommandIds.TOGGLE_COMMENT, PerlEditorActionIds.TOGGLE_COMMENT);
        wireAction(new CritiqueSourceAction(this), PerlEditorCommandIds.CRITIQUE_SOURCE, PerlEditorActionIds.CRITIQUE_SOURCE);
        ...
        wireAction(new OpenDeclarationAction(this), PerlEditorCommandIds.OPEN_SUB, PerlEditorActionIds.OPEN_SUB);
    ...
    protected final ISourceViewer createSourceViewer(Composite parent, IVerticalRuler ruler, int styles) {
        ...
        sourceViewer = new PerlSourceViewer(parent, ruler, fOverviewRuler, isOverviewRulerVisible(), styles);
    ...
    /**
     * The PerlEditor implementation of this AbstractTextEditor method adds any PerlEditor specific entries.
     */
    ...
    protected void handlePreferenceStoreChanged(PropertyChangeEvent event) {
        ...
        try {
            if (event.getProperty().equals(MarkOccurrencesPreferences.MARK_OCCURRENCES)) {
                boolean oldValue = event.getOldValue() != null ? Boolean.valueOf(event.getOldValue().toString()).booleanValue()
                        : false;

                boolean newValue = event.getOldValue() != null ? Boolean.valueOf(event.getNewValue().toString()).booleanValue()
                        : false;
    ...
    protected void initializeEditor() {
        ...
        setPreferenceStore(new ChainedPreferenceStore(new IPreferenceStore[] {
                PerlEditorPlugin.getDefault().getPreferenceStore(), this.getPreferenceStore() }));
    ...
    private void caretMoved() {
        ...
        if (outlinePage == null || source == null || source.getDocument() == null)
            return;
    ...
    private void installBracketInserter() {
        bracketInserter = new PerlBracketInserter(PerlEditorPlugin.getDefault().getLog());
        ...
        bracketInserter.setIsNeedByTalend(isEditable());
    }
    ...
    private void installSyntaxValidationThread() {
        ...
        validationThread.setDocument((IResource) ((IAdaptable) getEditorInput()).getAdapter(IResource.class), sourceViewer
                .getDocument());

        // Register the validation thread if automatic checking is enabled
        if (PerlEditorPlugin.getDefault().getSyntaxValidationPreference() && idleTimer != null) {
    ...
    private void reconfigureBracketInserter() {
        ...
        if (bracketInserter != null) {
            bracketInserter.setCloseBracketsEnabled(preferenceStore.getBoolean(PreferenceConstants.AUTO_COMPLETION_BRACKET1));
            bracketInserter.setCloseBracesEnabled(preferenceStore.getBoolean(PreferenceConstants.AUTO_COMPLETION_BRACKET2));
            bracketInserter.setCloseParensEnabled(preferenceStore.getBoolean(PreferenceConstants.AUTO_COMPLETION_BRACKET3));
            bracketInserter.setCloseAngularBracketsEnabled(preferenceStore
                    .getBoolean(PreferenceConstants.AUTO_COMPLETION_BRACKET4));
            bracketInserter.setCloseDoubleQuotesEnabled(preferenceStore.getBoolean(PreferenceConstants.AUTO_COMPLETION_QUOTE1));
            bracketInserter.setCloseSingleQuotesEnabled(preferenceStore.getBoolean(PreferenceConstants.AUTO_COMPLETION_QUOTE2));
        }
    }
    ...
    /**
     * Contains methods that provide access to internal workings of PerlEditor intended to be available only to
     * white-box test cases. Other clients should not make use of this interface.
     */
    ...
    /**
     * Tracks caret movements in order to update selection in the outline page. Implementation borrowed from
    ...
    /**
     * Tracks selection in the outline page in order to highlight subroutine declarations in source code.
     */
     ...
     * 
     * Instead of going to the start of a line it does the following: - if smart home/end is enabled and the caret is
     * after the line's first non-whitespace then the caret is moved directly before it; beginning of a comment ('#')
     * counts as whitespace - if the caret is before the line's first non-whitespace, the caret is moved to the
     * beginning of the line - if the caret is at the beginning of the line, see first case.
     */
    ...
    /**
     * Base class for actions that navigate to or select text up to the next word boundary.
     */
    protected abstract class WordNavigationAction extends TextNavigationAction {
        ...
        public final void run() {
            ...
            PerlPartitioner partitioner = (PerlPartitioner) document.getDocumentPartitioner();
            int docLength = document.getLength();

            run(viewer, document, position, partitioner, docLength);
        }

        protected abstract void run(ISourceViewer viewer, IDocument document, int position, PerlPartitioner partitioner,
                int docLength);
        ...
        protected final void setCaretPosition(final int position) {
            if (select) {
                ...
                getSourceViewer().getTextWidget().setCaretOffset(modelOffset2WidgetOffset(getSourceViewer(), position));
            }
        }
    }
    ...
    private final class NextWordAction extends WordNavigationAction {
         ...
         * {@link org.eclipse.swt.custom.ST}.
         */
        ...
        protected void run(ISourceViewer viewer, IDocument document, int position, PerlPartitioner partitioner, int length) {
            try {
                ...
                if (position < length && (document.getChar(position) == '\n' || document.getChar(position) == '\r')) {
                    // Make "next word" action at the end of a line go to the
                    // begining of
                    // the next line
                    ...
                    while (position < partitionEnd && !Character.isWhitespace(document.getChar(position)))
                        position++;

                    while (position < length && Character.isWhitespace(document.getChar(position))
            ...
            } catch (BadLocationException e) // should never occur
            {
                PerlEditorPlugin.getDefault().getLog().log(
                        new Status(IStatus.ERROR, PerlEditorPlugin.getPluginId(), IStatus.OK,
                                "An unexpected exception occurred in NextWordAction", e));
        ...
        protected void run(ISourceViewer viewer, IDocument document, int position, PerlPartitioner partitioner, int length) {
            try {
                ...
                if (document.getLineOffset(line) == position) {
                    ...
                    while (position > partitionStart && Character.isWhitespace(document.getChar(position - 1))
                            && document.getChar(position - 1) != '\n' && document.getChar(position - 1) != '\r')
                        position--;
            ...
            } catch (BadLocationException e) // should never occur
            {
                PerlEditorPlugin.getDefault().getLog().log(
                        new Status(IStatus.ERROR, PerlEditorPlugin.getPluginId(), IStatus.OK,
                                "An unexpected exception occurred in PreviousWordAction", e));
