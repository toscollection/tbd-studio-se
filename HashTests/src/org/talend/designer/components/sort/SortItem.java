 ...
 * purposes and without fee is hereby granted. Please refer to the file http://java.sun.com/copy_trademarks.html for
 * further important copyright and trademark information and to http://java.sun.com/licensing.html for further important
 ...
 * SUN SPECIFICALLY DISCLAIMS ANY EXPRESS OR IMPLIED WARRANTY OF FITNESS FOR HIGH RISK ACTIVITIES.
 */
...
/**
 * A simple applet class to demonstrate a sort algorithm. You can specify a sorting algorithm using the "alg" attribyte.
...
public class SortItem extends java.applet.Applet implements Runnable {
    ...
    /**
     * A generalization of high and low water marks.
     */
    int h[];
    ...
    /**
     * Another pause. Really, this could replace the others. But, I haven't done it yet. The different name is to remind
     * me to clean this up.
     * 
     * Andrew Kitchen 95/11/22
     */
    protected void apause(int H[]) {
        for (int i = 0; i < H.length; i++)
            h[i] = H[i];
        if (kicker != null) {
            repaint();
    ...
    /**
     * For a Thread to actually do the sorting. This routine makes sure we do not simultaneously start several sorts if
     * the user repeatedly clicks on the sort item. It needs to be synchronoized with the stop() method because they
