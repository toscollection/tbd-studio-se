...
public class RegExpView extends ViewPart {
        ...
        public void initMatches() {
            ...
            try {
                for (int i = bracketsInRegexp.size() - 1; i >= 0; i--) {
                    SubexpressionPos pos = (SubexpressionPos) bracketsInRegexp.get(i);
    ...
    public void createPartControl(Composite parent) {
        ...
        data.horizontalSpan = 4;
        matchText = new StyledText(panel, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
    ...
    private void createShortcutsMenu(IMenuManager mgr) {
        ...
        try {
            File shortcutsFile = new File(RegExpPlugin.getPlugInDir() + File.separator + "shortcuts");
        ...
        try {
            ...
            for (int i = 0; i < matches.length; i++) {
                ...
                for (int j = 1; j <= re.getNumSubs(); j++) {
                    ...
                    Display display = panel.getDisplay();
                    styleRange.foreground = display.getSystemColor(SWT.COLOR_WHITE);
                    ...
                    int offsetFromLine = styleRange.start
                            - matchText.getOffsetAtLine(matchText.getLineAtOffset(styleRange.start));
    ...
    public void setResultLabelImage(ImageDescriptor descr) {
        Image labelImage = new Image(resultImageLabel.getDisplay(), descr.getImageData());
                ...
                for (int i = 0; i < input.length(); i++) {
                    ...
                    if (!inBracket) {
                        ...
                        if (re.isMatch(character)) {
                            ...
                            debugInfo.addBracketPosition(bracketStart, result.length() - 1);
    ...
    private void showDebugResult(int position) {
        ...
        if (debugInfo == null) {
            ...
            position = 1;
        } else if (!debugInfo.getInput().equals(regExpText.getText()) || !debugInfo.getMatchString().equals(matchText.getText())) {
            buildDebugRegExp(regExpText.getText(), matchText.getText());
            position = 1;
        }
        ...
        int offsetFromLine = styleRangeRegExp.start
                - regExpText.getOffsetAtLine(regExpText.getLineAtOffset(styleRangeRegExp.start));
        ...
        if (position <= debugInfo.geMatchingBracketsCount()) {
            ...
            for (int i = 0; i < matches.length; i++) {
                ...
                offsetFromLine = styleRangeMatch.start
                        - matchText.getOffsetAtLine(matchText.getLineAtOffset(styleRangeMatch.start));
                matchText.setHorizontalIndex(offsetFromLine);
                matchText.redraw();
            }
    ...
    /**
     * @param state The state of the ingore checkbox, <code>true</code> or <code>false</code>
     */
    ...
    /**
     * @param state The state of the multiline checkbox, <code>true</code> or <code>false</code>
     */
