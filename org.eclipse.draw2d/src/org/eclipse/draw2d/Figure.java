...
public class Figure implements IFigure {
    ...
    private static final int FLAG_VALID = new Integer(1).intValue(), FLAG_OPAQUE = new Integer(1 << 1).intValue(),
            FLAG_VISIBLE = new Integer(1 << 2).intValue(), FLAG_FOCUSABLE = new Integer(1 << 3).intValue(),
            FLAG_ENABLED = new Integer(1 << 4).intValue(), FLAG_FOCUS_TRAVERSABLE = new Integer(1 << 5).intValue();

    static final int FLAG_REALIZED = 1 << 31;

    /**
     * The largest flag defined in this class. If subclasses define flags, they should declare them as larger than this
     * value and redefine MAX_FLAG to be their largest flag value.
     * <P>
    ...
    /**
     * Calls {@link #add(IFigure, Object, int)} with <code>null</code> as the constraint and -1 as the index.
     * 
    ...
    /**
     * Returns a descendant of this Figure such that the Figure returned contains the point (x, y), and is accepted by
     * the given TreeSearch. Returns <code>null</code> if none found.
     * 
    ...
    /**
     * Returns the deepest descendant for which {@link #isMouseEventTarget()} returns <code>true</code> or
     * <code>null</code> if none found. The Parameters <i>x</i> and <i>y</i> are absolute locations. Any Graphics
     * transformations applied by this Figure to its children during {@link #paintChildren(Graphics)} (thus causing the
     * children to appear transformed to the user) should be applied inversely to the points <i>x</i> and <i>y</i> when
     * called on the children.
     * 
     * @param x The X coordinate
     * @param y The Y coordinate
    ...
    /**
     * Searches this Figure's children for the deepest descendant for which {@link #isMouseEventTarget()} returns
     * <code>true</code> and returns that descendant or <code>null</code> if none found.
     * 
    ...
    /**
     * Notifies to all {@link CoordinateListener}s that this figure's local coordinate system has changed in a way which
     * affects the absolute bounds of figures contained within.
     * 
     * @since 3.1
     */
    ...
    /**
     * Notifies to all {@link FigureListener}s that this figure has moved. Moved means that the bounds have changed in
     * some way, location and/or size.
     * 
     * @since 3.1
     */
    ...
    /**
     * Fires both figuremoved and coordinate system changed. This method exists for compatibility. Some listeners which
     * used to listen for figureMoved now listen for coordinates changed. So to be sure that those new listeners are
     * notified, any client code which used called this method will also result in notification of coordinate changes.
     * 
     * @since 2.0
    ...
    /**
     * Notifies any {@link PropertyChangeListener PropertyChangeListeners} listening to this Figure that the boolean
     * property with id <i>property</i> has changed.
     * 
    ...
    /**
     * Notifies any {@link PropertyChangeListener PropertyChangeListeners} listening to this figure that the Object
     * property with id <i>property</i> has changed.
     * 
     * @param property The id of the property that changed
     * @param old The old value of the changed property
    ...
    /**
     * Notifies any {@link PropertyChangeListener PropertyChangeListeners} listening to this figure that the integer
     * property with id <code>property</code> has changed.
     * 
     * @param property The id of the property that changed
     * @param old The old value of the changed property
     * @param current The current value of the changed property
    ...
    /**
     * Returns this Figure's background color. If this Figure's background color is <code>null</code> and its parent is
     * not <code>null</code>, the background color is inherited from the parent.
     * 
    ...
    /**
     * Returns the smallest rectangle completely enclosing the figure. Implementors may return the Rectangle by
     * reference. For this reason, callers of this method must not modify the returned Rectangle.
     * 
    ...
    /**
     * Returns the border's Insets if the border is set. Otherwise returns NO_INSETS, an instance of Insets with all 0s.
     * Returns Insets by reference. DO NOT Modify returned value. Cannot return null.
     * 
    ...
    /**
     * Returns an Iterator over the listeners of type <i>clazz</i> that are listening to this Figure. If there are no
     * listeners of type <i>clazz</i>, an empty iterator is returned.
     * 
    ...
    /**
     * Returns <code>null</code> or the local background Color of this Figure. Does not inherit this Color from the
     * parent.
     * 
    ...
    /**
     * Returns <code>null</code> or the local font setting for this figure. Does not return values inherited from the
     * parent figure.
     * 
    ...
    /**
     * Returns <code>null</code> or the local foreground Color of this Figure. Does not inherit this Color from the
     * parent.
     * 
    ...
    /**
     * Returns <code>true</code> if revalidating this Figure does not require revalidating its parent.
     * 
     * @return <code>true</code> if revalidating this Figure doesn't require revalidating its parent.
     * @since 2.0
     */
    ...
    /**
     * Paints this Figure's children. The caller must save the state of the graphics prior to calling this method, such
     * that <code>graphics.restoreState()</code> may be called safely, and doing so will return the graphics to its
    ...
    /**
     * Paints this Figure's client area. The client area is typically defined as the anything inside the Figure's
     * {@link Border} or {@link Insets}, and by default includes the children of this Figure. On return, this method
    ...
    /**
     * Paints this Figure's primary representation, or background. Changes made to the graphics to the graphics current
     * state will not affect the subsequent calls to {@link #paintClientArea(Graphics)} and
     * {@link #paintBorder(Graphics)}. Furthermore, it is safe to call <code>graphics.restoreState()</code> within this
     * method, and doing so will restore the graphics to its original state upon entry.
     * 
     * @param graphics The Graphics used to paint
     * @since 2.0
     */
    ...
    /**
     * Removes the given child Figure from this Figure's hierarchy and revalidates this Figure. The child Figure's
     * {@link #removeNotify()} method is also called.
     * 
    ...
    /**
     * Sets the bounds of this Figure to the Rectangle <i>rect</i>. Note that <i>rect</i> is compared to the Figure's
     * current bounds to determine what needs to be repainted and/or exposed and if validation is required. Since
     * {@link #getBounds()} may return the current bounds by reference, it is not safe to modify that Rectangle and then
     * call setBounds() after making modifications. The figure would assume that the bounds are unchanged, and no layout
     * or paint would occur. For proper behavior, always use a copy.
     * 
    ...
    /**
     * Sets the direction of any {@link Orientable} children. Allowable values for <code>dir</code> are found in
    ...
    /**
     * Sets the orientation of any {@link Orientable} children. Allowable values for <i>orientation</i> are found in
     * {@link PositionConstants}.
     * 
    ...
    /**
     * Sets this figure to be valid if <i>value</i> is <code>true</code> and invalid otherwise.
     * 
