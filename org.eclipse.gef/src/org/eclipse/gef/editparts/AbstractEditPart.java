 ...
 * <P>
 * Since this is the default implementation of an interface, this document deals with proper sub-classing of this
 * implementation. This class is not the API. For documentation on proper usage of the public API, see the documentation
 ...
 * <P>
 * AbstractEditPart provides support for children. All AbstractEditPart's can potentially be containers for other
 * EditParts.
 */
public abstract class AbstractEditPart implements EditPart, RequestConstants, IAdaptable {
    ...
    /**
     * The left-most bit that is reserved by this class for setting flags. Subclasses may define additional flags
     * starting at <code>(MAX_FLAG << 1)</code>.
     */
    ...
    /**
     * Iterates over a <code>List</code> of EditPolcies, skipping any <code>null</code> values encountered.
     */
    ...
    /**
     * Activates this EditPart, which in turn activates its children and EditPolicies. Subclasses should <em>extend</em>
     * this method to add listeners to the model. Activation indicates that the EditPart is realized in an
     * EditPartViewer. <code>deactivate()</code> is the inverse, and is eventually called on all EditParts.
     * 
     ...
     * <OL>
     * <LI>The child is added to the {@link #children} List, and its parent is set to <code>this</code> <LI>
     * {@link #addChildVisual(EditPart, int)} is called to add the child's visual <LI>{@link EditPart#addNotify()} is
     * called on the child. <LI><code>activate()</code> is called if this part is active <LI><code>EditPartListeners
     * </code> are notified that the child has been added.
     * </OL>
     * <P>
    ...
    /**
     * Performs the addition of the child's <i>visual</i> to this EditPart's Visual. The provided subclasses
     * {@link AbstractGraphicalEditPart} and {@link AbstractTreeEditPart} already implement this method correctly, so it
    ...
    /**
     * Creates the initial EditPolicies and/or reserves slots for dynamic ones. Should be implemented to install the
     * inital EditPolicies based on the model's initial state. <code>null</code> can be used to reserve a "slot", should
    ...
    /**
     * Deactivates this EditPart, and in turn deactivates its children and EditPolicies. Subclasses should
     * <em>extend</em> this method to remove any listeners established in {@link #activate()}
     * 
    ...
    /**
     * This method will log a message to GEF's trace/debug system if the corresponding flag for EditParts is set to
     * true.
     * 
    ...
    /**
     * This method will log the message to GEF's trace/debug system if the corrseponding flag for FEEDBACK is set to
     * true.
     * 
    ...
    /**
     * Erases source feedback for the given <code>Request</code>. By default, this responsibility is delegated to this
     * part's <code>EditPolicies</code>. Subclasses should rarely extend this method.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
     * <td>It is recommended that feedback be handled by EditPolicies, and not directly by the EditPart.
     * </tr>
     * </table>
     * 
    ...
    /**
     * Erases target feedback for the given <code>Request</code>. By default, this responsibility is delegated to this
     * part's EditPolicies. Subclasses should rarely extend this method.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
    ...
    /**
     * Notifies <code>EditPartListeners</code> that this EditPart has been activated.
     */
    ...
    /**
     * Notifies <code>EditPartListeners </code> that this EditPart has been deactivated.
     */
    ...
    /**
     * Returns the <code>AccessibleEditPart</code> adapter for this EditPart. The <B>same</B> adapter instance must be
     * used throughout the editpart's existance. Each adapter has a unique ID which is registered during
     * {@link #register()}. Accessibility clients can only refer to this editpart via that ID.
     * 
    ...
    /**
     * Subclasses should rarely extend this method. The default implementation combines the contributions from each
     * installed <code>EditPolicy</code>. This method is implemented indirectly using EditPolicies.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
     * <td>It is recommended that Command creation be handled by EditPolicies, and not directly by the EditPart.
     * </tr>
     * </table>
     * 
    ...
    /**
     * Used internally to iterate over the installed EditPolicies. While EditPolicy slots may be reserved with
     * <code>null</code>, the iterator only returns the non-null ones.
     * 
    ...
    /**
     * Returns the boolean value of the given flag. Specifically, returns <code>true</code> if the bitwise AND of the
     * specified flag and the internal flags field is non-zero.
     * 
    ...
    /**
     * Returns a <code>List</code> containing the children model objects. If this EditPart's model is a container, this
     * method should be overridden to returns its children. This is what causes children EditParts to be created.
     * <P>
    ...
    /**
     * Returns the <code>EditPart</code> which is the target of the <code>Request</code>. The default implementation
     * delegates this method to the installed EditPolicies. The first non-<code>null</code> result returned by an
     * EditPolicy is returned. Subclasses should rarely extend this method.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
     * <td>It is recommended that targeting be handled by EditPolicies, and not directly by the EditPart.
     * </tr>
     * </table>
     * 
    ...
    /**
     * Subclasses should extend this method to handle Requests. For now, the default implementation does not handle any
     * requests.
     * 
    ...
    /**
     * Refreshes all properties visually displayed by this EditPart. The default implementation will call
     * {@link #refreshChildren()} to update its structural features. It also calls {@link #refreshVisuals()} to update
     * its own displayed properties. Subclasses should extend this method to handle additional types of structural
     * refreshing.
     */
    public void refresh() {
        refreshVisuals();
    ...
    /**
     * Updates the set of children EditParts so that it is in sync with the model children. This method is called from
     * {@link #refresh()}, and may also be called in response to notification from the model. This method requires
     * linear time to complete. Clients should call this method as few times as possible. Consider also calling
     * {@link #removeChild(EditPart)} and {@link #addChild(EditPart, int)} which run in constant time.
     * <P>
     * The update is performed by comparing the exising EditParts with the set of model children returned from
     * {@link #getModelChildren()}. EditParts whose models no longer exist are {@link #removeChild(EditPart) removed}.
    ...
    /**
     * Refreshes this EditPart's <i>visuals</i>. This method is called by {@link #refresh()}, and may also be called in
     * response to notifications from the model. This method does nothing by default. Subclasses may override.
     */
    protected void refreshVisuals() {
    }

    /**
     * Registers itself in the viewer's various registries. If your EditPart has a 1-to-1 relationship with a visual
     * object and a 1-to-1 relationship with a model object, the default implementation should be sufficent.
     * 
     * @see #unregister()
    ...
    /**
     * Registers the <i>model</i> in the {@link EditPartViewer#getEditPartRegistry()}. Subclasses should only extend
    ...
    /**
     * Registers the <i>visuals</i> in the {@link EditPartViewer#getVisualPartMap()}. Subclasses should override this
     * method for the visual part they support. {@link AbstractGraphicalEditPart} and {@link AbstractTreeEditPart}
     * already do this.
     */
    ...
    /**
     * Removes a child <code>EditPart</code>. This method is called from {@link #refreshChildren()}. The following
     * events occur in the order listed:
     * <OL>
     * <LI><code>EditPartListeners</code> are notified that the child is being removed <LI><code>deactivate()</code> is
     * called if the child is active <LI>{@link EditPart#removeNotify()} is called on the child. <LI>
     * {@link #removeChildVisual(EditPart)} is called to remove the child's visual object. <LI>The child's parent is set
     * to <code>null</code>
     * </OL>
     * <P>
    ...
    /**
     * Removes the childs visual from this EditPart's visual. Subclasses should implement this method to support the
     * visual type they introduce, such as Figures or TreeItems.
     * 
     ...
     * <UL>
     * <LI>deselecting this EditPart if selected <LI>setting the Viewer's focus to <code>null</code> if this EditPart
     * has <i>focus</i> <LI>{@link #unregister()} this EditPart
     * </UL>
     * <P>
     * In addition, <code>removeNotify()</code> is called recursively on all children EditParts. Subclasses should
     * <em>extend</em> this method to perform any additional cleanup.
     * 
    ...
    /**
     * Moves a child <code>EditPart</code> into a lower index than it currently occupies. This method is called from
     * {@link #refreshChildren()}.
     * 
    ...
    /**
     * Sets the value of the specified flag. Flag values are decalared as static constants. Subclasses may define
     * additional constants above {@link #MAX_FLAG}.
     * 
    ...
    /**
     * sets the selected state for this EditPart. This method should rarely be overridden. Instead, EditPolicies that
     * are selection-aware will listen for notification of this property changing.
     * 
    ...
    /**
     * Shows or updates source feedback for the given <code>Request</code>. By default, this responsibility is delegated
     * to this part's EditPolicies. Subclasses should rarely extend this method.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
    ...
    /**
     * Shows or updates target feedback for the given <code>Request</code>. By default, this responsibility is delegated
     * to this part's EditPolicies. Subclasses should rarely extend this method.
     * <P>
     * <table>
     * <tr>
    ...
    /**
     * Returns <code>true</code> if this <code>EditPart</code> understand the given <code>Request</code>. By default,
     * this responsibility is delegated to this part's installed EditPolicies.
     * <P>
     * <table>
     * <tr>
     * <td><img src="../doc-files/important.gif"/>
    ...
    /**
     * Undoes any registration performed by {@link #register()}. The provided base classes will correctly unregister
     * their visuals.
     */
    ...
    /**
     * Unregisters the <i>model</i> in the {@link EditPartViewer#getEditPartRegistry()}. Subclasses should only extend
    ...
    /**
     * Unregisters the <i>visuals</i> in the {@link EditPartViewer#getVisualPartMap()}. Subclasses should override this
     * method for the visual part they support. {@link AbstractGraphicalEditPart} and {@link AbstractTreeEditPart}
     * already do this.
     */
