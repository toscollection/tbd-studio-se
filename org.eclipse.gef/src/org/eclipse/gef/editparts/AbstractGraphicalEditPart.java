 ...
 * <P>
 * This is an implementation class, and the documentation here is targeted at subclassing this class. Callers of public
 * API should refer to the interface's documentation.
 */
public abstract class AbstractGraphicalEditPart extends AbstractEditPart implements GraphicalEditPart {
    ...
    private boolean drawerFigure = false;
    ...
    /**
     * A default implementation of {@link AccessibleEditPart}. Subclasses can extend this implementation to get base
    ...
    /**
     * The default implementation of {@link AccessibleAnchorProvider} returned in {@link #getAdapter(Class)}. This
     * implementation creates an accessible location located along the right edge of the EditPart's Figure.
     * 
     * @since 2.0
     */
    ...
    /**
     * Extends {@link AbstractEditPart#activate()} to also activate all <i>source</i> ConnectionEditParts.
     * 
    ...
    /**
     * Adds a <i>source</i> ConnectionEditPart at the specified index. This method is called from
     * {@link #refreshSourceConnections()}. There should be no reason to call or override this method. Source connection
     * are created as a result of overriding {@link #getModelSourceConnections()}.
     * <P>
     * {@link #primAddSourceConnection(ConnectionEditPart, int)} is called to perform the actual update of the
     * {@link #sourceConnections} <code>List</code>. The connection will have its source set to <code>this</code>.
     * <P>
    ...
    /**
     * Adds a <i>target</i> ConnectionEditPart at the specified index. This method is called from
     * {@link #refreshTargetConnections()}. There should be no reason to call or override this method. Target connection
     * are created as a result of overriding {@link #getModelTargetConnections()}.
     * <P>
     * {@link #primAddTargetConnection(ConnectionEditPart, int)} is called to perform the actual update of the
     * {@link #targetConnections} <code>List</code>. The connection will have its target set to <code>this</code>.
     * <P>
     * Finally, all {@link NodeListener}s are notified of the new connection.
     * 
     * @param connection Connection being added
    ...
    /**
     * Creates a {@link ConnectionEditPart} for the given model. Similar to {@link AbstractEditPart#createChild(Object)}
     * . This method is called indirectly during {@link #refreshSourceConnections()}, and
     * {@link #refreshTargetConnections()}.
     * <P>
     * The default implementation goes to the EditPartViewer's {@link EditPartFactory} to create the connection. This
     * method should not be overridden unless factories are not being used.
     * 
    ...
    /**
     * Creates the <code>Figure</code> to be used as this part's <i>visuals</i>. This is called from
     * {@link #getFigure()} if the figure has not been created.
     * 
     * @return a Figure
     */
     ...
     * {@link EditPartViewer#getEditPartRegistry() EditPart registry} and returns it if one is found. Otherwise,
     * {@link #createConnection(Object)} is called to create a new ConnectionEditPart. Override this method only if you
    ...
    /**
     * Extends {@link AbstractEditPart#deactivate()} to also deactivate the source ConnectionEditParts. Subclasses
     * should <em>extend</em> this method to remove any listeners added in {@link #activate}.
     * 
     ...
     * {@link #removeSourceConnection(ConnectionEditPart)}. There is no reason for subclasses to call or override this
     * method.
     * 
     ...
     * {@link #removeTargetConnection(ConnectionEditPart)}. There is no reason for subclasses to call or override this
     * method.
     * 
     * @param connection <code>ConnectionEditPart</code> being added as child.
     * @param index Position child is being added into.
     ...
     * {@link #addSourceConnection(ConnectionEditPart, int)}. There is no reason for subclasses to call or override this
     * method.
     * 
     * @param connection <code>ConnectionEditPart</code> being added as child.
     * @param index Position child is being added into.
     ...
     * {@link #addTargetConnection(ConnectionEditPart, int)}. There is no reason for subclasses to call or override this
     * method.
     * 
     * @param connection <code>ConnectionEditPart</code> being added as child.
     * @param index Position child is being added into.
    ...
    /**
     * Extends {@link AbstractEditPart#getAdapter(Class)} to handle additional adapter types. Currently, these types
     * include {@link AccessibleHandleProvider} and {@link AccessibleAnchorProvider}. Subclasses should <em>extend</em>
     * this method to support additional adapter types, or to replace the default provided adapaters.
     * 
    ...
    /**
     * The default implementation calls {@link #createFigure()} if the figure is currently <code>null</code>.
     * 
    ...
    /**
     * A convenience method for obtaining the specified layer from the <code>LayerManager</code>.
     * 
    ...
    /**
     * Returns the <code>List</code> of the connection model objects for which this EditPart's model is the
     * <b>source</b>. {@link #refreshSourceConnections()} calls this method. For each connection model object,
     * {@link #createConnection(Object)} will be called automatically to obtain a corresponding
    ...
    /**
     * Returns the <code>List</code> of the connection model objects for which this EditPart's model is the
     * <b>target</b>. {@link #refreshTargetConnections()} calls this method. For each connection model object,
     * {@link #createConnection(Object)} will be called automatically to obtain a corresponding
     * {@link ConnectionEditPart}.
     * <P>
     * Callers must not modify the returned List.
    ...
    /**
     * Removes the specified source <code>ConnectionEditPart</code> from the {@link #sourceConnections} List. This
     * method is called from {@link #removeSourceConnection(ConnectionEditPart)}. Subclasses should not call or override
     * this method.
     * 
    ...
    /**
     * Removes the specified target <code>ConnectionEditPart</code> from the {@link #targetConnections} List. This
     * method is called from {@link #removeTargetConnection(ConnectionEditPart)}. Subclasses should not call or override
     * this method.
     * 
     * @param connection Connection to remove.
     */
    ...
    /**
     * Extends {@link AbstractEditPart#refresh()} to refresh two additional structural features: <i>source</i> and
     * <i>target</i> connections. Subclasses should probably override {@link AbstractEditPart#refreshVisuals()} instead
     * of this method.
     * 
    ...
    /**
     * Updates the set of <i>source</i> ConnectionEditParts so that it is in sync with the model source connections.
     * This method is called from {@link #refresh()}, and may also be called in response to notification from the model.
     * <P>
     * The update is performed by comparing the exising source ConnectionEditParts with the set of model source
     * connections returned from {@link #getModelSourceConnections()}. EditParts whose model no longer exists are
     * {@link #removeSourceConnection(ConnectionEditPart) removed}. New models have their ConnectionEditParts
     * {@link #createConnection(Object) created}. Subclasses should override <code>getModelSourceChildren()</code>.
     * <P>
    ...
    /**
     * Updates the set of <i>target</i> ConnectionEditParts so that it is in sync with the model target connections.
     * This method is called from {@link #refresh()}, and may also be called in response to notification from the model.
     * <P>
     * The update is performed by comparing the exising source ConnectionEditParts with the set of model source
     * connections returned from {@link #getModelTargetConnections()}. EditParts whose model no longer exists are
     * {@link #removeTargetConnection(ConnectionEditPart) removed}. New models have their ConnectionEditParts
     * {@link #createConnection(Object) created}. Subclasses should override <code>getModelTargetChildren()</code>.
     * <P>
     * This method should <em>not</em> be overridden.
     */
    ...
    /**
     * Registers the EditPart's Figure in the Viewer. This is what makes it possible for the Viewer to map a mouse
     * location to an EditPart.
     * 
    ...
    /**
     * Extends {@link AbstractEditPart#removeNotify()} to cleanup <code>ConnectionEditParts</code>.
     * 
    ...
    /**
     * Removes the given connection for which this EditPart is the <B>source</b>. <BR>
    ...
    /**
     * Removes the given connection for which this EditPart is the <B>target</b>. <BR>
     * Fires notification. <BR>
    ...
    /**
     * Moves a source <code>ConnectionEditPart</code> into a lower index than it currently occupies. This method is
     * called from {@link #refreshSourceConnections()}.
     * 
     * @param connection the ConnectionEditPart
    ...
    /**
     * Moves a target <code>ConnectionEditPart</code> into a lower index than it currently occupies. This method is
     * called from {@link #refreshTargetConnections()}.
     * 
     * @param connection the ConnectionEditPart
     * @param index the new index
     */
    ...
    protected void setFigure(IFigure figure) {
        // if (this.figure != null && this.figure.getBackgroundColor() != null)
        // this.figure.getBackgroundColor().dispose();
        // if (this.figure != null && this.figure.getForegroundColor() != null)
        // this.figure.getForegroundColor().dispose();
        this.figure = figure;
    }
