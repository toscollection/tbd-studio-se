...
/**
 * An abstract implementation of TransferDropTargetListener for use with {@link EditPartViewer}s. The Viewer's
 * <code>Control</code> should be the Drop target. In order to communicate with EditParts in a consistent way,
 * DropTargetEvents are processed into {@link Request Requests}.
 * <P>
 * Dropping is inherently a targeting interaction. This class handles calculating the <i>target</i> EditPart. It also
 * handles common targeting behavior, such as interacting with the target EditPart or its ancestors to achieve things
 * like auto-scroll/auto-expose.
 */
public abstract class AbstractTransferDropTargetListener implements TransferDropTargetListener {
    ...
    /**
     * Constructs a new AbstractTransferDropTargetListener and sets the EditPartViewer. If you use this constructor, you
     * must set the Transfer yourself using {@link #setTransfer(Transfer)}.
     * 
    ...
    /**
     * Creates and returns the <code>Request</code> that will be sent to the targeted EditPart. Subclasses can override
     * to create specialized requests.
     * 
    ...
    /**
     * Stores the information about the current DropTargetEvent. This method may not be called on the listener, because
     * the listener may not be made active until after the mouse has entered the drop target.
     * 
    ...
    /**
     * Stores the information about the current DropTargetEvent and then calls <code>unload()</code>. Subclasses should
     * override {@link #unload()} to perform actions for this event. For some reason, SWT also calls
     * <code>dragLeave()</code> when the actual drop is performed, even though the mouse has not left the drop target.
     * 
    ...
    /**
     * Stores the information about the current DropTargetEvent and then calls <code>handleDragOperationChanged()</code>
     * . Subclasses should override {@link #handleDragOperationChanged()} to perform actions for this event.
     * 
    ...
    /**
     * Stores the information about the current DropTargetEvent and then calls <code>handleDragOver()</code>. Subclasses
     * should override {@link #handleDragOver()} to perform actions for this event.
     * 
    ...
    /**
     * Stores the information about the current DropTargetEvent and then calls {@link #handleDrop()}, followed by
     * {@link #unload()}. Subclasses should override these methods to perform actions for this event.
     * 
    ...
    /**
     * Stores the current <code>DropTargetEvent</code> and does nothing. By default, the drop is accepted.
     * 
    ...
    /**
     * Calls <code>eraseTargetFeedback(Request)</code> on the current <i>target</i>, using the target Request. Does
     * nothing if there is no target, or if the target has not been requested to show target feedback.
     */
    ...
    /**
     * Returns the current mouse location, as a {@link Point}. The location is relative to the control's client area.
     * 
    ...
    /**
     * Returns a Collection of {@link EditPart EditParts} that are to be excluded when searching for the target
     * EditPart.
     * 
    ...
    /**
     * Called when the user changes the Drag operation. By default, target feedback is erased. The target Request and
     * target EditPart are updated, and target feedback is re-displayed on the new target.
     */
    ...
    /**
     * Called whenever the User drags over the target. By default, the target Request and target EditPart are updated,
     * feedback is shown, and auto-expose occurs.
     */
    ...
    /**
     * Updates the target Request and target EditPart, and performs the drop. By default, the drop is performed by
     * asking the target EditPart for a Command using the target Request. This Command is then executed on the
     * CommandStack.
     * <P>
     * If there is no target EditPart or no executable Command, the event's <code>detail</code> field is set to
     * <code>DND.DROP_NONE</code>.
     */
    ...
    /**
     * Returns <code>true</code> if this TransferDropTargetListener is enabled for the specified
     * <code>DropTargetEvent</code>. By default, this is calculated by comparing the event's
     * {@link DropTargetEvent#dataTypes dataTypes} with the <code>Transfer's</code> supported types (
     * {@link Transfer#isSupportedType(TransferData)}). If a dataType is supported, an attempt is made to find a
     * <i>target</i> <code>EditPart</code> at the current drop location. If a target <code>EditPart</code> is found,
     * <code>true</code> is returned, and the DropTargetEvent's {@link DropTargetEvent#currentDataType} is set to the
     * dataType that matched.
     * 
     * @param event the DropTargetEvent
     * @return <code>true</code> if this TransferDropTargetListener is enabled for the given DropTargetEvent
     */
    ...
    /**
     * Returns <code>true</code> if {@link #isEnabled(DropTargetEvent)} is determined by asking the potential target for
     * a Command.
     * 
     * @return <code>true</code> if the target will be queried for a <code>Command</code>
     * @since 3.1
     */
    ...
    /**
     * Determines if the target editpart should be asked for a Command during {@link #isEnabled(DropTargetEvent)}. For
     * most DND operations, the data is not available, thus asking for a command would not make sense. The default value
     * is <code>false</code>.
     * 
    ...
    /**
     * Sets the <i>target</i> <code>EditPart</code>. If the target is changing, {@link #handleExitingEditPart()} is
     * called before the target changes, and {@link #handleEnteredEditPart()} is called afterwards.
     * 
    ...
    /**
     * Asks the target <code>EditPart</code> to show target feedback if it is not <code>null</code>.
     * 
    ...
    /**
     * Tests whether the given event's location is different than the previous event's location, and sets the remembered
     * location to the current event's location.
     * 
     * @param event
     * @return boolean
     */
    ...
    /**
     * Updates the active {@link AutoexposeHelper}. Does nothing if there is still an active helper. Otherwise, obtains
     * a new helper (possible <code>null</code>) at the current mouse location and calls
