 ...
 * {@link org.eclipse.gef.EditPart EditParts}. The viewer is responsible for the editpart
 * lifecycle. Editparts have <i>visuals</i>, such as <code>TreeItems</code> or
 * <code>Figures</code>, which are hosted by the viewer and its control. The viewer
 * provides targeting of editparts via their visuals.
 * 
 * <P>
 * A viewer is a {@link org.eclipse.jface.viewers.ISelectionProvider}. It maintains a list
 * of selected editparts. The last member of this list is the <i>primary</i> member of the
 * selection. The list should never be empty; when no editparts are selected, the viewer's
 * contents editpart is used.
 * 
 * <P>
 * A viewer is populated by setting its <i>contents</i>. This can be done by passing the
 * model corresponding to the contents. The viewer's {@link
 * org.eclipse.gef.EditPartFactory EditPartFactory} is then used to create the contents
 * editpart, and add it to the <i>root</i> editpart. Alternatively, the contents editpart
 ...
 * used, it may be common to replace the root editpart. See implementations of {@link
 * org.eclipse.gef.RootEditPart}.
 * 
 * <P>
 * An editpart's lifecycle is managed by the viewer. When the Viewer is realized, meaning
 * it has an SWT <code>Control</code>, it activates its root, which in turn activates all
...
/**
 * Returns the <code>KeyHandler</code> for this viewer.  The KeyHandler is sent KeyEvents
 * by the currently active <code>Tool</code>. This is important, because only the current
 ...
 * becomes the primary selection.  Fires selection changed to {@link
 * org.eclipse.jface.viewers.ISelectionChangedListener}s.
...
/**
 * Sets the context <code>MenuManager</code> for this viewer. The MenuManager will be
