...
public class DrawerFigure extends Figure {
     ...
     * Constructor
     * 
     * @param control The Control of the LWS to which this Figure belongs (it is used to display the drawer header with
     * an EditPartTipHelper, if the header is not completely visible). It can be <code>null</code> (the tip won't be
     * displayed).
     */
    public DrawerFigure(final Control control) {
        /*
         * A PaletteToolbarLayout is being used here instead of a ToolbarLayout so that the ScrollPane can be stretched
         * to take up vertical space. This affects selection and appearance (background color).
         */
         ...
         * @TODO:Pratik
         * 
         * There is a bug here. Right-click on the name pop-up for the header of a drawer figure in the palette. This
         * will hide the pop-up. Right-click again, this time on the collapse toggle, to bring up the drawer's context
         * menu. Now, left-click on the collapse toggle. The context menu will disappear, the name pop-up will
         * re-appear, and the drawer will collapse/expand. If the drawer was in such a position, that
         * collapsing/expanding it will cause its header to move, the name pop-up will now be floating over where the
         * collapse toggle used to be, but is not anymore. To fix this, you can detect the left mouse click on the
         * collapse toggle and hide the name pop-up then. The problem is that when you click on the collapseToggle after
         * the context menu has been brought up, it does not fire a mouse pressed event. The listener below, that is
         * commented out for now, is never notified.
         */
    ...
    /**
     * @return The content pane for this figure, i.e. the Figure to which children can be added.
     */
    ...
    public Dimension getMinimumSize(int wHint, int hHint) {
        /*
         * Fix related to Bug #35176 The figure returns a minimum size that is of at least a certain height, so as to
         * prevent each drawer from getting too small (in which case, the scrollbars cover up the entire available
         * space).
         */
        if (isExpanded()) {
    ...
    protected void handleExpandStateChanged() {
        if (isExpanded()) {
            ...
            for (Iterator iterator = getContentPane().getChildren().iterator(); iterator.hasNext();) {
                ...
                if (child instanceof PinnablePaletteStackFigure && !((PinnablePaletteStackFigure) child).isPinnedOpen()) {
    ...
    /**
     * @return <code>true</code> if the drawer is expanded and is pinned (i.e., it can't be automatically collapsed)
     */
    ...
    /**
     * Pins or unpins the drawer. The drawer can be pinned open only when it is expanded. Attempts to pin it when it is
     * collapsed will do nothing.
     * 
