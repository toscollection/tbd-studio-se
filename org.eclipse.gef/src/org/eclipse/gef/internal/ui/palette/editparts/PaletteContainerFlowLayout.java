...
/**
 * Extends <code>FlowLayout</code> to allow the pane of a pinnable stack in
 * icon mode to occupy the row following the row in which the icon of the
 * palette stack header appears.
 * 
 * @author crevells
 * @since 3.4
 */
...
protected Dimension calculatePreferredSize(IFigure container, int wHint,
    ...
    for (int i = 0; i < children.size(); i++) {
        ...
        if (child instanceof PinnablePaletteStackFigure
            ...
            // orientation. Ignore the other hint (by setting it to -1). NOTE:
            // The children of the parent figure will then be asked to ignore
            // that hint as well.
            if (isHorizontal()) {
                hHint = -1;
            } else {
...
protected void layoutRow(IFigure parent) {
    ...
    for (int j = 0; j < data.rowCount; j++) {
        ...
        if (child instanceof PinnablePaletteStackFigure
            ...
            child.setBounds(new Rectangle(data.area.x, data.area.y + data.rowY,
                data.area.width, data.rowHeight + expandedPaneHeight));
        }
    }
