...
/**
 * The FlyoutPaletteComposite is used to show a flyout palette alongside another control. The flyout palette auto-hides
 * (thus maximizing space) when not in use, but can also be pinned open if so desired. It will only be visible when the
 * PaletteView is not.
 * 
 * @author Pratik Shah
 * @since 3.0
...
public class FlyoutPaletteComposite extends Composite {
    ...
    /**
     * One of the two possible initial states of the flyout palette. This is the default one. When in this state, only
     * the flyout palette's sash is visible.
     */
    public static final int STATE_COLLAPSED = 2;

    /**
     * One of the two possible initial states of the flyout palette. When in this state, the flyout palette is
     * completely visible and pinned open so that it doesn't disappear when the user wanders away from the flyout.
     */
    ...
    /*
     * Fix for Bug# 71525 transferFocus is used to transfer focus from the button in the vertical sash title to the
     * button in the horizontal paletteComposite title. When either button is pressed it is set to true, and when either
     * the sash or the paletteComposite gets notified of the change in state, they transfer the focus to their button if
     * this flag is set to true and if that button is visible.
     */
    ...
    public FlyoutPaletteComposite(Composite parent, int style, IWorkbenchPage page, PaletteViewerProvider pvProvider,
        ...
        addListener(SWT.Resize, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                /*
                 * @TODO:Pratik Sometimes, the editor is resized to 1,1 or 0,0 (depending on the platform) when the
                 * editor is closed or maximized. We have to ignore such resizes. See Bug# 62748
                 */
    ...
    /**
     * This is a convenient method to get a default FlyoutPreferences object. The returned FlyoutPreferences does not
     * save any changes made to the given {@link Preferences Preferences}. It's upto the owner plugin to
     ...
     * @return a default implementation of FlyoutPreferences that stores the settings in the given Preferences
     * @since 3.2
     */
    ...
    public void layout(boolean changed) {
        ...
        /*
         * Fix for Bug# 65892 Laying out only when necessary helps reduce flicker on GTK in the case where the flyout
         * palette is being resized past its maximum size.
         */
    ...
    private void layoutComponentsWest(Rectangle area, int sashWidth, int pWidth) {
        if (isInState(STATE_COLLAPSED)) {
            ...
            paletteContainer.setBounds(area.x, area.y, pWidth, area.height);
            sash.setBounds(area.x + pWidth, area.y, sashWidth, area.height);
            graphicalControl.setBounds(area.x + pWidth + sashWidth, area.y, area.width - sashWidth - pWidth, area.height);
        }
    }
     ...
     * {@link PaletteViewer#saveState(IMemento)} -- active tool, drawer expansion state, drawer pin state, etc.) will be
     * maintained when switching between the two viewers. Providing an external viewer, although recommended, is
     * optional.
     * 
    ...
    /**
     * Sets the control along the side of which the palette is to be displayed. The given Control should be a child of
     * this Composite. This method should only be invoked once.
     * 
    ...
    /*
     * @TODO:Pratik For 4.0, change the parameter of this method to be EditpartViewer instead of GraphicalViewer.
     */
    /**
     * This method hooks a DropTargetListener that collapses the flyout patette when the user drags something from the
     * palette and moves the cursor to the primary viewer's control. If the auto-hide feature of the palette is to work
     * properly when dragging, this method should be called before any other drop target listeners are added to the
     * primary viewer.
     * 
    ...
    /*
     * If the given state is invalid (as could be the case when FlyoutPreferences.getPaletteState() is invoked for the
     * first time), it will be defaulted to STATE_COLLAPSED.
     */
    private void setState(int newState) {
        /*
         * Fix for Bug# 69617 and Bug# 81248 FlyoutPreferences.getPaletteState() could return an invalid state if none
         * is stored. In that case, we use the default state: STATE_COLLAPSED.
         */
        ...
        /*
         * Fix for Bug# 63901 When the flyout collapses, if the palette has focus, throw focus to the graphical control.
         * That way, hitting ESC will still deactivate the current tool and load the default one. Note that focus is
         * being set on RulerComposite and not GraphicalViewer's control. But this is okay since RulerComposite passes
         * the focus on to its first child, which is the graphical viewer's control.
         */
    ...
    public interface FlyoutPreferences {
        ...
        /**
         * Should return {@link PositionConstants#EAST} or {@link PositionConstants#WEST}. Any other int will be ignored
         * and the default dock location (EAST) will be used instead.
         * 
        ...
        /**
         * When there is no saved state, this method can return any non-positive int (which will result in the palette
         * using the default state -- collapsed), or {@link FlyoutPaletteComposite#STATE_COLLAPSED}, or
        ...
        /**
         * When there is no saved width, this method can return any int (preferrably a non-positive int). Returning a
         * non-positive int will cause the palette to be sized to the default size, whereas returning a postive int will
         * find the closest match in the valid range (>= minimum and <= maximum)
         * 
        ...
        /**
         * This method is invoked when the flyout palette's dock location is changed. The provided dock location should
         * be persisted and returned in {@link #getDockLocation()}.
         * 
        ...
        /**
         * This method is invoked when the flyout palette's state is changed (the new state becomes the default). The
        ...
        /**
         * This method is invoked when the flyout palette is resized. The provided width should be persisted and
         * returned in {@link #getPaletteWidth()}.
         * 
    ...
    private class Sash extends Composite {
        ...
        private void handleSashDragged(int shiftAmount) {
            int newSize = paletteContainer.getBounds().width + (dock == PositionConstants.EAST ? -shiftAmount : shiftAmount);
    ...
    private class TitleDragManager extends MouseAdapter implements Listener, MouseTrackListener {
        ...
        public void mouseHover(MouseEvent e) {
            /*
             * @TODO:Pratik Mouse hover events are received if the hover occurs just before you finish or cancel the
             * drag. Open a bugzilla about it?
             */
            if (isInState(STATE_COLLAPSED))
                setState(STATE_EXPANDED);
        }
    ...
    private class ButtonCanvas extends Canvas {
        ...
        private class ArrowButton extends Button {
            ...
            protected void layout() {
                ...
                triangle.setBounds(new org.eclipse.draw2d.geometry.Rectangle(clientArea.getCenter().getTranslated(
                        -ARROW_SIZE.width / 2, -ARROW_SIZE.height / 2), ARROW_SIZE));
            }
    ...
    private class TitleCanvas extends Canvas {
        ...
        public void setFont(Font font) {
            ...
            if (isVisible()) {
                /*
                 * If this canvas is in the sash, we want the FlyoutPaletteComposite to layout (which will cause the
                 * sash to be resized and laid out). However, if this canvas is in the paletteContainer, the
                 * paletteContainer's bounds won't change, and hence it won't layout. Thus, we also invoke
                 * getParent().layout().
                 */
    ...
    /**
     * Default implementation of FlyoutPreferences that stores the flyout palette settings in the given Preferences.
     * 
     * @author Pratik Shah
     * @since 3.2
     */
    private static class DefaultFlyoutPreferences implements FlyoutPreferences {
        ...
        /*
         * There's no need to set the default for these properties since the default-default of 0 for ints will suffice.
         */
