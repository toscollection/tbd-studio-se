...
public class GraphicalViewerImpl extends AbstractEditPartViewer implements GraphicalViewer {
    ...
    /**
     * This method is invoked when this viewer's control loses focus. It removes focus from the
     * {@link AbstractEditPartViewer#focusPart focusPart}, if there is one.
     * 
     * @param fe the focusEvent received by this viewer's control
     */
    ...
    public EditPart findObjectAtExcluding(Point pt, Collection exclude, final Conditional condition) {
        ...
        IFigure figure = getLightweightSystem().getRootFigure().findFigureAt(pt.x, pt.y, new ConditionalTreeSearch(exclude));
        EditPart part = null;
    ...
    /**
     * Reveals the specified editpart by using {@link ExposeHelper}s. A bottom-up scan through the parent-chain is
     * performed, looking for expose helpers along the way, and asking them to expose the given editpart.
     * 
    ...
    /**
     * Extends the drag source to handle figures which handle MouseDown events, thereby aborting any DragDetect
     * callbacks.
     * 
    ...
    protected void setDragSource(DragSource source) {
        ...
        class TheLastListener extends DragSourceAdapter {
            ...
            public void dragStart(DragSourceEvent event) {
                ...
                if (event.doit) {
                    ...
                    /*
                     * The mouse down that came before the dragstart, or the dragstart event itself, may have caused
                     * selection or something that needs to be painted. paints will not get processed during DND, so
                     * flush.
                     */
                    flush();
                }
        ...
        /*
         * The DragSource may be set to null if there are no listeners. If there are listeners, this should bethe last
         * listener because all other listeners are hooked in super().
         */
        if (source != null)
    ...
    private static class MouseWheelDelegateHandler implements MouseWheelHandler {
        ...
        public void handleMouseWheel(Event event, EditPartViewer viewer) {
            ...
            do {
                MouseWheelHelper helper = (MouseWheelHelper) part.getAdapter(MouseWheelHelper.class);
                if (helper != null)
    ...
    public Control creatToolControl() {
        return null;
    }
