...
/**
 * A Graphical Viewer implementation which uses a {@link org.eclipse.draw2d.FigureCanvas} for native scrolling. Because
 * the scrolling is handled natively, the root editpart should not contain a {@link org.eclipse.draw2d.ScrollPane}
 * figure. Do not use root editparts which provide scrollpane figures, such as <code>GraphicalRootEditPart</code>.
 * <P>
 * The RootEditPart for a ScrollingGraphicalViewer may contain a Viewport. If it does, that viewport will be set as the
 * FigureCanvas' viewport. FigureCanvas has certain requirements on the viewport figure, see
...
public class ScrollingGraphicalViewer extends GraphicalViewerImpl {
    ...
    /**
     * Convenience method which types the control as a <code>FigureCanvas</code> . This method returns <code>null</code>
    ...
    /**
     * Extends the superclass implementation to scroll the native Canvas control after the super's implementation has
     * completed.
     * 
