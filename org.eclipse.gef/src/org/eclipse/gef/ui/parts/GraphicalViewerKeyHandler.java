...
/**
 * An extended KeyHandler which processes default keystrokes for common navigation in a GraphicalViewer. This class can
 * be used as a KeyHandler too; Unrecognized keystrokes are sent to the super's implementation. This class will process
 * key events containing the following:
 * <UL>
...
public class GraphicalViewerKeyHandler extends KeyHandler {
    ...
    /**
     * Given a connection on a node, this method finds the next (or the previous) connection of that node.
     * 
    ...
    /**
     * Given an absolute point (pStart) and a list of EditParts, this method finds the closest EditPart (except for the
     * one to be excluded) in the given direction.
     * 
     * @param siblings List of sibling EditParts
     * @param pStart The starting point (must be in absolute coordinates) from which the next sibling is to be found.
    ...
    /**
     * Figures' navigation points are used to determine their direction compared to one another, and the distance
     * between them.
     * 
    ...
    /**
     * Returns the cached node. It is possible that the node is not longer in the viewer but has not been garbage
     * collected yet.
     */
    ...
    /**
     * Returns the list of editparts which are conceptually at the same level of navigation as the currently focused
     * editpart. By default, these are the siblings of the focused part.
     * <p>
    ...
    /**
     * This method is invoked when the user presses the space bar. It toggles the selection of the EditPart that
     * currently has focus.
     * 
