...
/**
 * This class displays a red cross on the standard jobs, when a tHBaseInput or Output with custom timestamp option
 * enabled
 * use a timestamp column which is not of Long type.
 */
public class HbaseTimestampTypeProblem implements NodeProblem {

    private static final List<String> COMPONENTS = Arrays.asList("tHBaseInput", "tHBaseOutput", "tMapRDBInput",
            "tMapRDBOutput");
        ...
        if ((metadatas != null) && (metadatas.size() > 0)) {
            ...
            if (metadata != null) {
                List<Map<String, String>> mapping =
                        (List<Map<String, String>>) node.getElementParameter(mappingVariable).getValue();
                ...
                if (localTimestampColumn != null
                        && JavaTypesManager.getJavaTypeFromId(localTimestampColumn.getTalendType()) != JavaTypesManager.LONG) {
