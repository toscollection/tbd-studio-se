...
/**
 * Created by Marvin Wang on Mar. 31, 2012 for Talend Oozie Scheduler setup
 * dialog.
 */
public class TOozieSettingDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		settingComposite = new OozieSettingComposite(parent, SWT.BORDER, this) {
			 ...
			 * 
			 * @see
			 * org.talend.oozie.scheduler.ui.OozieSettingComposite#preInitialization
			 * ()
			 */
			@Override
	...
	public void setOozieEndPointValue(String oozieEndPointValue) {
		propertyMap.put(EOozieParameterName.OOZIE_END_POINT.getName(),
				oozieEndPointValue);
	...
	protected void okPressed() {
		this.setHadoopDistributionValue(settingComposite
				.getHadoopDistributionValue());
		this.setHadoopVersionValue(settingComposite.getHadoopVersionValue());
		if ("Built-in".equals(settingComposite.getPropertyTypeCombo().getText())) {
			...
			/*List<ElementParameter> list=(List<ElementParameter>) process.getElementParameters();
			for (EOozieParameterName oozieParam : EOozieParameterName.values()) {
	            if(!"REPOSITORY_CONNECTION_ID".equals(oozieParam.getName())&&!"JOBID_FOR_OOZIE".equals(oozieParam.getName())&&!"HADOOP_APP_PATH".equals(oozieParam.getName())) {
	            	ElementParameter param = new ElementParameter(process);
			...
			for (EOozieParameterName param : EOozieParameterName.values()) {
				if (!"REPOSITORY_CONNECTION_ID".equals(param.getName())
						&& !"JOBID_FOR_OOZIE".equals(param.getName())
						&& !"HADOOP_APP_PATH".equals(param.getName())) {
					PropertyChangeCommand pcc = new PropertyChangeCommand(
							process, param.getName(), propertyMap.get(param
									.getName()));
					command.add(pcc);
				}
			}
